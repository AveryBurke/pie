...
- *change order*: Rearrange the slices, rings, and color encodings by dragging the items.
- *select new columns: Use the dropdown menus to choose new columns for these encodings.
- *filter*: select a column against which to filter the data, using the drop down, and use the check boxes to select values to filter out
